...
> **Estado esperado hoy (tras el hotfix 4E-C11): 1081 pruebas · 1081 pasan · 0 FALLAN · 0 omitidas (140 suites, 27 archivos; `25` añade 30).** Antes de 4E-C11 (4E-C9): 1051 · 135 suites · 26 archivos. `24` (17) arranca el **bundle compilado** `api-server/dist/index.mjs` como proceso real
...
| `16-alcance-honesto.test.mjs` | **Alcance de 3.13.0**: Mi Trabajo (nota visible, subtitulo, listas vacias, manifest, build en /tmp) no promete trabajo asignado; copy de «Usuarios y equipo»; `apiUrl` con el backend de produccion y sin secretos (antes de 4E-C11 fijaba lo contrario); README y CHANGELOG (version, enlaces existentes, cifras de versionado, limitaciones) | 23 |
...
| `25-config-produccion-apiurl.test.mjs` | **HOTFIX PRE-TAG 4E-C11 · configuracion de produccion del Taller.** Mira el `supabase-config.js` **REAL** (no uno sintetico): `apiUrl` presente, HTTPS, solo el origen y EXACTAMENTE el backend aprobado; 17 mutantes del archivo real (vacio, sin la propiedad, `http://`, barra final, ruta, placeholder, localhost, host sintetico, otro servicio, clave `service_role`, `habilitado:false`, …) que el verificador debe **detectar**; el CLI `verificar-config-produccion.mjs` (salida 0/1, sin imprimir la clave anon); la pantalla real «Usuarios y equipo» con ese `apiUrl` (ya no sale «Falta indicar la dirección del servidor»; pide a `<backend>/api/admin/usuarios`), el control con `apiUrl` vacio (el aviso exacto que vio el QA) y cajero/mecanico rechazados; el build de Mi Trabajo copia la misma config. **Falla contra `8eea52c`** (7 casos). Motivo: el QA de 3.13.0 inyectaba un `apiUrl` sintetico y nadie miraba el archivo publicado (`DEPLOYMENT_CONFIGURATION_GAP`) | 30 |

`verificar-config-produccion.mjs` (no es una prueba: no lo ejecuta `node --test`) es la **puerta del paso de publicacion** del Taller: `node pruebas/multiusuario/verificar-config-produccion.mjs [archivo-o-URL]` comprueba el archivo que se va a subir o la URL ya publicada (un GET). Sin argumentos, el versionado.
